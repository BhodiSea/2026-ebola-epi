...
- Read `lib/supabase/proxy.ts` to understand cookie refresh; don't
  bypass it.
- `getUser()` (not `getSession()`) when authoritative identity is needed —
  it round-trips to the Auth server and revalidates.

## Proxy (`proxy.ts` at apps/web root — Next.js 16 routing middleware)

- Cookie refresh for Supabase Auth lives here. Calls `lib/supabase/proxy.ts`.
- **Never use `String.raw\`...\`` in the matcher** — Next.js static analysis
  can't evaluate tagged template literals; use plain strings with `\\.`.
...
This file is now at `apps/web/app/CLAUDE.md` — the monorepo migration
(Phase 0) is complete. `@`-imports reference `../.claude/references/…`.
